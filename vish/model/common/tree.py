...
class LabelHierarchyTree:
    ...
    def get_elements_at_depth(self, depth: int, names=True):
        ...
        return format_list_elements(elements, names)
    ...
    def get_immediate_children(self, tag: str, names=False):
        ...
        return format_list_elements(imm_children, names)
    ...
    def getall_leaves(self, tag: str, names=True):
        ...
        for node in e.iter():
            if self.is_leaf(node):
                leaves.append(node)
        return format_list_elements(leaves, names)


if __name__ == "__main__":
    ht = LabelHierarchyTree("vish/data/cifar10.xml")
